...
for s in seeds:
    ...
    for sample_name in sample_list:
        ...
        model = GraphST.GraphST(adata,random_seed=int(s), device=device)
        adata = model.train()
        radius = 50
    ...
    # clustering
       ...
       # np.save(f"{dir_output}/domain_refined_{sample_name}.npy",adata.obs['domain'])
        ...
        sc.pl.embedding(adata, basis="spatial", color="domain",palette='magma' , show=False,legend_loc='none', title="GraphST: ARI=%.4f"%ARI)
